    ...
    <header className="fixed inset-x-0 top-0 z-50 border-b border-gray-200 bg-white/95 shadow-sm backdrop-blur-md">
      <div className="mx-auto flex h-20 max-w-[1440px] items-center justify-between gap-5 px-4">
        <Link href="/" className="flex min-h-11 shrink-0 flex-col items-start justify-center">
          ...
          <span className="mt-1 text-[11px] font-bold tracking-wide text-gray-700">{header.tagline}</span>
        </Link>

        <nav aria-label="Điều hướng chính" className="hidden items-center gap-1 xl:flex">
          {navigation.map((item) => (
            <Link key={item.href} href={item.href} className="inline-flex min-h-11 items-center rounded-lg px-3 text-sm font-bold text-gray-800 transition-colors hover:bg-orange-50 hover:text-[#D83B00]">
              {item.label}
            </Link>
          ))}
        </nav>
        ...
        <div className="hidden shrink-0 items-center gap-3 md:flex">
          ...
          >
            <ZaloIcon className="h-4 w-4" fill="white" /> {header.ctaZaloLabel}
          </a>
        </div>
        ...
        <button
          ...
          className="inline-flex h-12 w-12 items-center justify-center rounded-xl border border-gray-300 bg-white text-gray-900 xl:hidden"
        >
      ...
      <nav
        ...
        className={`${open ? "flex" : "hidden"} max-h-[calc(100vh-5rem)] flex-col overflow-y-auto border-t border-gray-200 bg-white px-4 pb-6 pt-3 xl:hidden`}
      >
        {navigation.map((item) => (
          <Link key={item.href} href={item.href} onClick={() => setOpen(false)} className="flex min-h-12 items-center border-b border-gray-100 px-2 text-base font-bold text-gray-900">
            {item.label}
          </Link>
        ))}
