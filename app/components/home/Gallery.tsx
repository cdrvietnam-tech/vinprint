...
export default function Gallery() {
  const [images, setImages] = useState<ManagedMediaItem[]>(DEFAULT_MEDIA_COLLECTIONS.gallery);
  const tabs = ["Tất cả", "Mỹ phẩm", "Thực phẩm", "Đồ uống", "Trà - Cafe", "Handmade", "Pet"];

  useEffect(() => {
    fetch("/api/media/collections?collection=gallery", { cache: "no-store" })
      .then(async (response) => response.ok ? response.json() as Promise<{ items: ManagedMediaItem[] }> : null)
      .then((result) => {
        if (result?.items.length) setImages(result.items);
      })
      .catch(() => undefined);
  }, []);

  return (
    <div id="mau-thuc-te" className="scroll-mt-24 bg-white rounded-[32px] p-6 lg:p-8 flex flex-col h-full shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] border border-gray-100">
      <div className="flex items-center justify-between gap-2 mb-6 shrink-0 w-full">
        ...
        <button className="inline-flex min-h-11 items-center justify-center gap-1.5 rounded-full bg-[#4933D4] px-4 py-2.5 text-xs font-bold text-white shadow-sm transition-colors hover:bg-[#3725AE] sm:px-5 sm:text-sm">
          Xem thêm {images.length}+ mẫu <span className="hidden xs:inline">thực tế</span> <ArrowRight className="w-3 h-3 sm:w-4 sm:h-4" />
        </button>
      </div>
